...
class L1Regularizer(_Regularizer):
    ...
    def __init__(self, model, lambda_reg=1e-6):
...
class ElasticNetRegularizer(_Regularizer):
    ...
    def __init__(self, model, lambda_reg=1e-6, alpha_reg=0.01):
...
class GroupSparseLassoRegularizer(_Regularizer):
    ...
    def regularized_param(self, param_weights, reg_loss_function):
        reg_loss_function = self.lambda_reg * (
            self.reg_l2_l1.regularized_param(param_weights=param_weights, reg_loss_function=reg_loss_function) + self.reg_l1.regularized_param(param_weights=param_weights, reg_loss_function=reg_loss_function))

        return reg_loss_function

    def regularized_all_param(self, reg_loss_function):
        reg_loss_function = self.lambda_reg * (
            self.reg_l2_l1.regularized_all_param(reg_loss_function=reg_loss_function) + self.reg_l1.regularized_all_param(reg_loss_function=reg_loss_function))
...
class GroupLassoRegularizer(_Regularizer):
    ...
    def regularized_param(self, param_weights, reg_loss_function, group_name='input_group'):
        if group_name == 'input_group':
            ...
            print(f'The group {group_name} is not supported yet. Please try one of this: [input_group, hidden_group, bias_group]')
        return reg_loss_function
    ...
    def __bias_groups_reg(bias_weights):
        return GroupLassoRegularizer.__grouplasso_reg(groups=bias_weights, dim=-1)  # or 0 i dont know yet
